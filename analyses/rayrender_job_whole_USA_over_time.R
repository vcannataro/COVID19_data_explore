...
dates_to_pull <- seq(as.Date("2020-03-10"),max(polygons$date),by = 1)

for(i in 1:dates_to_pull) {
  ...
  scene <- generate_ground(depth=0,spheresize=5000, 
                           material=diffuse(color="#000000",
                                            noise=1/10,
                                            noisecolor = "#654321")) %>%
    add_object(extruded_polygon(test_first, center = F,data_column_top = "roll_new_cases",
                                scale_data = 1/max(polygons$roll_new_cases,na.rm = T)*5,
                                material= diffuse(color="forestgreen"))) %>%
  ...
  render_scene(scene = scene, lookat = c(72,0,38),lookfrom = c(60,9,35), 
               parallel=T,samples=500,fov=70,width=500,
               height=500,filename=glue::glue("output_data/figures/tests/frames/USA_diffuse_over_time{i}.png"))
  # 
...
av::av_encode_video(glue::glue("output_data/figures/tests/frames/USA_diffuse_over_time{1:(frames-1)}.png"),
                    framerate=20, output = "output_data/figures/tests/USA_rotate_diffuse_over_time.mp4")
